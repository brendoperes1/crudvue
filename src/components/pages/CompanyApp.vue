            ...
            <table class="min-w-full divide-y divide-gray-300">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Número de funcionários</th>
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="company in companys" :key="company.name">
                  ...
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ company.numberOfEmployees }}</td>
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ company.country }}</td>
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ company.category }}</td>
                  <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                    <a href="#" class="text-indigo-600 hover:text-indigo-900"
                      >Editar<span class="sr-only">, {{ company.name }}</span></a
                    >
                  </td>
                </tr>
              </tbody>
...
export default {
  ...
  async created() {
    try {
      const res =  await axios.get('http://localhost:3000/companys');
